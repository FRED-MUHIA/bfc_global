...
@section('content')
    <section class="py-16 md:py-20">
        <div class="container-base">
            <div class="mx-auto max-w-3xl">
                <div class="mb-8">
                    <p class="text-sm font-bold uppercase tracking-[0.15em] text-ember">Event Registration</p>
                    <h1 class="mt-3 text-4xl leading-tight md:text-5xl">{{ $event['title'] }}</h1>
                    <p class="mt-4 text-slate/75">{{ $event['date'] }} • {{ $event['time'] }} • {{ $event['location'] }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('event.register.store') }}" class="glass-panel overflow-hidden p-6 md:p-8" data-multi-step-form>
                    @csrf
                    ...
                    <div class="grid overflow-hidden">
                        ...
                        <section data-step class="hidden grid gap-4">
                            ...
                            <label class="grid gap-2">
                                <span class="text-sm font-bold text-pine">Organization</span>
                        ...
                        <section data-step class="hidden grid gap-4">
                            ...
                            <div class="rounded-2xl border border-sand bg-white p-4">
                                <p class="text-sm font-bold uppercase tracking-[0.12em] text-ember">Event</p>
                                <p class="mt-2 font-semibold text-pine">{{ $event['title'] }}</p>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const storageKey = `event-registration-${form.event_slug.value}`;
            let current = Number(localStorage.getItem(`${storageKey}-step`) || 0);

            const inputs = Array.from(form.querySelectorAll('input, textarea')).filter((input) => input.name && input.type !== 'hidden');
            ...
            const render = () => {
                ...
                if (current === steps.length - 1) {
                    review.innerHTML = inputs.map((input) => `
                        <div>
            ...
            const validateCurrent = () => {
                const fields = Array.from(steps[current].querySelectorAll('input, textarea'));
